...
saves one `PendingHoldAttempt` (seat ID + key) through `PendingHoldAttemptStore` before
network I/O; production uses UserDefaults and tests inject an in-memory store. If the
response is ambiguous, the app keeps the same pair and offers a retry. Details and trade-
offs are in [ADR-014](../docs/decisions/014-use-idempotency-for-hold-creation.md) and
[ADR-015](../docs/decisions/015-persist-pending-hold-attempt-locally.md).
...
execution does not yet prove runtime behavior on the minimum iOS version. Confirmation is
not yet connected to the hold-success screen.
